...
RISK_THRESHOLDS = {
    "LOW":      0.0,
    "MODERATE": 7.5,    # moderate: 7.5–15 mm/3h
    "HIGH":     15.0,   # heavy:   15–30 mm/3h
    "EXTREME":  30.0,   # extreme: >30 mm/3h
}
...
def assign_flood_risk(df: pd.DataFrame) -> pd.DataFrame:
    ...
    conditions = [
        r24 < 15,                         # LOW      (< 15 mm/24h)
        (r24 >= 15)  & (r24 < 50),        # MODERATE
        (r24 >= 50)  & (r24 < 100),       # HIGH
        r24 >= 100,                        # EXTREME
    ]
    labels = [0, 1, 2, 3]
...
BASE_FEATURE_COLS = [
    "rain_3h","rain_12h","rain_24h","rain_48h",
...
def train_models(X, y, feature_names):
    ...
    cv_scores = cross_val_score(rf_pipe, X, y, cv=5,
                                scoring="f1_weighted", n_jobs=-1)
    print(f"  Random Forest 5-fold CV F1: {cv_scores.mean():.4f} ± {cv_scores.std():.4f}\n")
...
def detect_extreme_events(df: pd.DataFrame) -> pd.DataFrame:
    """
    Flag extreme rainfall events using IQR + Z-score methods.
    Returns dataframe with outlier flags.
    """
    rain = df["rain_3h"].fillna(0)

    # IQR method
    Q1, Q3 = rain.quantile(0.25), rain.quantile(0.75)
    IQR = Q3 - Q1
    ...
    df = df.copy()
    df["rain_zscore"]      = (rain - mean_r) / (std_r if std_r > 0 else 1)
    df["iqr_outlier"]      = rain > iqr_upper
    df["zscore_outlier"]   = df["rain_zscore"].abs() > z_threshold
    df["extreme_event"]    = df["iqr_outlier"] | df["zscore_outlier"]

    # Severity tier for extreme events
    df["extreme_tier"] = "normal"
    df.loc[df["extreme_event"], "extreme_tier"] = "extreme"
    df.loc[rain > RISK_THRESHOLDS["EXTREME"], "extreme_tier"] = "catastrophic"

    extremes = df[df["extreme_event"]]
    print(f"{'─'*60}")
    print("  OUTLIER / EXTREME EVENT DETECTION")
    print(f"{'─'*60}")
    print(f"  IQR upper fence   : {iqr_upper:.2f} mm/3h")
    print(f"  Z-score threshold : ±{z_threshold}")
    print(f"  Extreme events    : {extremes['iqr_outlier'].sum()} (IQR) | "
          f"{extremes['zscore_outlier'].sum()} (Z-score) | "
          f"{df['extreme_event'].sum()} combined")
    print(f"  Catastrophic (>30 mm/3h): {(rain > 30).sum()} events\n")
...
def plot_rainfall_timeline(df, output_dir):
    ...
    for thresh, color, label in [
        (15,  "#f1c40f55", "Moderate"),
        (50,  "#e67e2255", "High"),
        (100, "#e74c3c55", "Extreme"),
    ]:
...
def plot_extreme_events(df, output_dir):
    """Scatter plot of extreme rainfall events with Z-scores."""
    ...
    zscore  = df.get("rain_zscore",   pd.Series(0.0,  index=df.index))
    ...
    ax2.scatter(t[normal_mask], zscore[normal_mask],
                c="#8b949e", s=3, alpha=0.3, label="Normal")
    ax2.scatter(t[extreme], zscore[extreme],
                c="#ff7b72", s=25, alpha=0.9, zorder=5, label="Outlier")
    ax2.axhline(3.0,  color="#e74c3c", linestyle="--", linewidth=1,
                label="+3σ threshold")
    ax2.axhline(-3.0, color="#e74c3c", linestyle="--", linewidth=1)
    ax2.set_ylabel("Z-score", color="white", fontsize=10)
    ax2.set_xlabel("Date", color="white", fontsize=10)
    ax2.set_title("Z-Score of 3h Rainfall", color="#8b949e", fontsize=11)
...
def export_predictions(df, output_dir):
    ...
    cols = ["datetime","station_id","rain_3h","rain_12h","rain_24h","rain_48h",
            ...
            "extreme_event","extreme_tier","rain_zscore"]
...
def export_extreme_events(df, output_dir):
    ...
    cols = ["datetime","station_id","rain_3h","rain_zscore","flood_risk_label",
